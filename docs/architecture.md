...
11. Clip-wide shoulder and hip overlap identifies side and oblique-side body views using a smooth confidence range. When strong, one robust orthogonal body frame keeps the shoulder and hip girdles level and parallel, projects their centerline into the sagittal plane, and centers the rigid head there. A robust clip-wide ankle width then softly constrains lateral leg spread, distributing corrections across each thigh and shin without changing their visible sagittal directions.
12. Low image-space motion identifies likely planted hands; their ground-plane coordinates provide a reach-limited target for a two-bone IK solve. Simultaneous left/right support contacts share a forward/back coordinate in ambiguous side views while preserving hand width.
